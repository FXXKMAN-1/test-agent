...
3. 选择模型 Provider 和具体模型（默认 DeepSeek V3）
4. 可选：拖动「最大步数」滑块调整上限（建议 40-60）
5. 点击「▶ 开始测试」

> **💡 提示**：测试目标越具体越好。不要只写"测试登录"，而是写清楚"打开某个地址，输入什么账号密码，期望看到什么"。

### 场景二：批量测试（从文档导入）

适用于多场景回归测试，支持 Excel / CSV / Markdown 格式。

**Excel 格式示例：**

| 用例编号 | 用例名称 | 测试步骤 | 预期结果 | 优先级 |
|----------|---------|---------|---------|--------|
| TC001 | 登录验证 | 打开后台 admin/123456 登录 | 进入首页 | 高 |
| TC002 | 新增产品 | 点击新增，填写产品名"大厨"，价格1000 | 保存成功 | 高 |
| TC003 | 搜索功能 | 搜索"大厨"，检查结果列表 | 至少出现1条 | 中 |

**Markdown 格式示例：**

```markdown
## TC001 登录验证
打开后台 https://admin.example.com
用 admin / 123456 登录
确认跳转到首页
- 预期: 成功进入首页
- 优先级: 高
...
如果你用的是 DeepSeek/OpenAI 以外的 API：

1. 进入「**设置**」页
2. 点击「+ 添加」
3. 填写：
   - **名称**：如"硅基流动"
   - **Provider ID**：如 `siliconflow`
   - **API Base URL**：如 `https://api.siliconflow.cn/v1`
   - **API Key**：你的密钥
   - **模型列表**：如 `Qwen/Qwen2.5-7B-Instruct`
4. 保存后，在新建测试页的 Provider 下拉中就能选到了

> 任何兼容 OpenAI `/v1/chat/completions` 协议的 API 都可以接入。

### 查看报告和导出

- 测试完成后自动跳转到报告页
- 报告包含：Agent 结论、每步操作详情、所有截图
- 点击「📤 导出 HTML」保存为独立文件，可在任意浏览器打开
...
| 问题 | 解决 |
|:-----|:------|
| 测试跑不完，提示"超过最大步数" | 把最大步数调大（如 80），或把测试目标拆得再细分 |
| Agent 反复点同一个地方 | 这是页面元素不好定位，换一种描述方式 |
| 浏览器没启动 | 首次启动会自动下载 Chromium（~130MB），等待完成即可 |
| API Key 报错 | 检查 Key 是否正确、余额是否充足、baseURL 是否带 `/v1` 结尾 |
